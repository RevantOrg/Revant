 ...
 *
 * @param args 4=keep only alignments of this length or bigger.
 */
public class PAF2LAshow {
...
//int[] ids = new int[] {13,576,5895,704,1175,10475,11915,1309,1352,2336,2468,3091,3125,3371,7225,4154,5512,5763,6697,6944,7628,9904,12384,16321,20631,45707};
//Arrays.sort(ids);
		...
		while (str!=null)  {
			...
			if (!sequenceNames.containsKey(tokens[0])) {
				sequenceNames.put(tokens[0],idGenerator++);
//if (Arrays.binarySearch(ids,0,ids.length,idGenerator-1)>=0) System.err.println((idGenerator-1)+" == "+tokens[0]);
			}
			if (!sequenceNames.containsKey(tokens[5])) {
				sequenceNames.put(tokens[5],idGenerator++);
//if (Arrays.binarySearch(ids,0,ids.length,idGenerator-1)>=0) System.err.println((idGenerator-1)+" == "+tokens[5]);
			}
			lastAlignment++;
			str=br.readLine();
		}
