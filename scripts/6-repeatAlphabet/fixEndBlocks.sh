...
UNIQUE_MODE="1"; OPEN_MODE="1"; MULTI_MODE="0"  # We do not use open blocks
rm -f ${TMPFILE_PATH}*
...
for K in $(seq 2 ${MAX_K}); do
	...
	for FILE in $(find -s ${INPUT_DIR} -name "${TMPFILE_NAME}-1-*"); do
		...
		kmersThread ${K} ${FILE} ${TMPFILE_PATH}-${K}-kmers-${THREAD_ID} &
	done
	wait
	...
	wait
	N_FIXED="0"; N_FIXABLE="0";
	for FILE in $(find -s ${INPUT_DIR} -name "${TMPFILE_NAME}-counts-${K}-*"); do
		N_FIXED=$(( ${N_FIXED} + $(cut -d , -f 1 ${FILE}) ))
		N_FIXABLE=$(( ${N_FIXED} + $(cut -d , -f 2 ${FILE}) ))
	done
	echo "Disambiguated ${N_FIXED} read ends out of ${N_FIXABLE} fixable"
done
