...
4. **Refining the reference sequences** - To build an accurate representation of each repeat, all its fragment sequences should be star-aligned to its reference sequence. However, the current reference sequence might not align to several fragments. This step builds a graph whose nodes are fragments (including those that compose the current reference), and whose edges are containment and overlap relations induced by all pairwise alignments between fragments.
